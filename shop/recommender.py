...
class Recommender(object):
    ...
    def product_bought(self, products):
        ...
        for product_id in product_ids:
            for with_id in product_ids:
                # get the other products bough with each product
    ...
    def suggest_products_for(self, products, max_results=6):
        ...
        if len(products) == 1:
            ...
            # generate temoporary key
            flat_ids = ''.join([str(id) for id in products_ids])
            tmp_key = f'tmp_{flat_ids}'
            # multiple products, combine scores of all products store the resulting sorted set in
            # a temporary key
